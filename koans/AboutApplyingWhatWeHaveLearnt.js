...
describe("About Applying What We Have Learnt", function() {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", function () {
      ...
      productsICanEat = _(products).filter(function (pizza) {
      	return (!_(pizza.ingredients).any(function(ingredient){return ingredient === 'mushrooms'}) && !pizza.containsNuts)
      });
      expect(productsICanEat.length).toBe(1);
  });
  ...
  it("should count the ingredient occurrence (functional)", function () {
	...
	ingredientCount = _(products).chain()
	  			...
	  			.flatten()
	  			.reduce(functions(s, i) {return ingredientCount[i] = (ingredientCount[i] || 0) + 1})
    expect(ingredientCount['mushrooms']).toBe(2);
  });
